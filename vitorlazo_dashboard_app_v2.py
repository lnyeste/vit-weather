...
def get_pure_live_weather(field, day_idx):
    ...
    # Hivatalos, szabványos URL az Open-Meteo globális repülési adatbázisához
    ...
    params = {
        ...
        "wind_speed_unit": "kmh",
        "timezone": "Europe/Budapest",  # FIX KÉNYSZERÍTETT MAGYAR IDŐZÓNA, nincs több index-eltolódás!
        "forecast_days": 3
    }
    ...
    try:
        ...
        if response.status_code != 200:
            st.error(f"❌ Hálózati hiba: Az időjárási szerver nem válaszolt (HTTP Kód: {response.status_code}).")
            st.stop()
            
        res = response.json()
        if "hourly" not in res or "temperature_2m" not in res["hourly"]:
            st.error("❌ Szerver hiba: Hibás vagy üres adatcsomag érkezett a műholdról.")
            st.stop()
            
        # Kiszámítjuk a fix, magyar időzóna szerinti pontos indexeket (Naponként 24 óra, 10:00 és 20:00 között)
        ...
        # Ha az időzóna-vágás után sincs elég adat, azt azonnal jelezzük
        if not hourly_temps or len(hourly_temps) < 11:
            st.error("❌ Hiba: Az időjárási idősor beolvasása sikertelen. Kérlek frissítsd az oldalt!")
            st.stop()
        ...
        st.error(f"❌ Kapcsolódási hiba: A felhőszerver nem tudott adatot letölteni. Ok: {str(e)}")
        st.stop()

    # 41 darab negyedórás lépés (10:00 - 20:00) lineáris interpolációval (Nulla véletlenszerű szám)
    for i in range(41):
        ...
        # Precíz lineáris interpoláció az órás mérések között
